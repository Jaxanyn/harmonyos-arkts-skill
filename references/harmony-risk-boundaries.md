...
Previously granted explicit authorization counts; ask only for the uncovered action. Read-only discovery and bounded diagnostic reads are not mutations. Build output generated by an authorized command does not need a second approval for each artifact. Never print secret values or full signing configuration when identifying protected surfaces.
